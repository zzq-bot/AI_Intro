...
public class OthelloState implements State {
	...
	private float starDifferential() {
		float diff  = 0;
		short[] corners = new short[4];
		...
		for(int i=0;i<4;i++) {
			if(corners[i]==0&&stars[i]!=0) {
				diff+= stars[i]==2? 1:-1;
			}
		}
		return diff;
	}
	private float cDifferential() {
		float diff  = 0;
		short[] corners = new short[4];
		...
		corners[2] = getSpotOnLine(hBoard[dimension - 1], (byte)0);
		corners[3] = getSpotOnLine(hBoard[dimension - 1], (byte)(dimension - 1));
		cs[0] = getSpotOnLine(hBoard[0],(byte)1);
		cs[1] = getSpotOnLine(hBoard[1],(byte)(0));
		cs[2] = getSpotOnLine(hBoard[0],(byte)(dimension-2));
		cs[3] = getSpotOnLine(hBoard[1],(byte)(dimension-1));
	...
	private float aDifferential() {
		float diff  = 0;
		short[] as = new short[8];
		as[0] = getSpotOnLine(hBoard[0],(byte)2);
		as[1] = getSpotOnLine(hBoard[2],(byte)(0));
		as[2] = getSpotOnLine(hBoard[0],(byte)(dimension-3));
		as[3] = getSpotOnLine(hBoard[2],(byte)(dimension-1));
		as[4] = getSpotOnLine(hBoard[dimension-3],(byte)(0));
		as[5] = getSpotOnLine(hBoard[dimension-3],(byte)(dimension-1));
		as[6] = getSpotOnLine(hBoard[dimension-1],(byte)2);
		as[7] = getSpotOnLine(hBoard[dimension-1],(byte)(dimension-3));
		for(short a:as)	if (a != 0) diff += a == 2 ? 1 : -1;
		return diff;
	}
	
	private float bDifferential() {
		float diff  = 0;
		short[] bs = new short[8];
		bs[0] = getSpotOnLine(hBoard[0],(byte)3);
		bs[1] = getSpotOnLine(hBoard[0],(byte)(dimension-4));
		bs[2] = getSpotOnLine(hBoard[3],(byte)(0));
		bs[3] = getSpotOnLine(hBoard[3],(byte)(dimension-1));
		bs[4] = getSpotOnLine(hBoard[dimension-4],(byte)(0));
		bs[5] = getSpotOnLine(hBoard[dimension-4],(byte)(dimension-1));
		bs[6] = getSpotOnLine(hBoard[dimension-1],(byte)3);
		bs[7] = getSpotOnLine(hBoard[dimension-1],(byte)(dimension-4));
		for(short b:bs)	if (b != 0) diff += b == 2 ? 1 : -1;
		return diff;
	}
	private float blockDifferential() {
		float diff = 0;
		int whites = 0;
		short [] blocks = new short[6];
		//1
		blocks[0] = getSpotOnLine(hBoard[0],(byte)2);
		blocks[1] = getSpotOnLine(hBoard[0],(byte)3);
		blocks[2] = getSpotOnLine(hBoard[0],(byte)4);
		blocks[3] = getSpotOnLine(hBoard[0],(byte)5);
		blocks[4] = getSpotOnLine(hBoard[0],(byte)1);
		blocks[5] = getSpotOnLine(hBoard[0],(byte)5);
		for(int i=0;i<4;i++) {
			if(blocks[i]!=0)
				whites += blocks[i]==2 ? 1 : -1;
		}
		if(whites==4) {
			diff+=1;
		if((blocks[4]==1&&blocks[5]==0)||(blocks[4]==0&&blocks[5]==1))
			diff -=2;
		}
		
		whites = 0;
		blocks[1] = getSpotOnLine(hBoard[dimension-1],(byte)2);
		blocks[2] = getSpotOnLine(hBoard[dimension-1],(byte)3);
		blocks[3] = getSpotOnLine(hBoard[dimension-1],(byte)4);
		blocks[4] = getSpotOnLine(hBoard[dimension-1],(byte)5);
		for(int i=0;i<4;i++) {
			if(blocks[i]!=0)
				whites += blocks[i]==2 ? 1 : -1;
		}
		...
		whites = 0;
		blocks[1] = getSpotOnLine(hBoard[2],(byte)0);
		blocks[2] = getSpotOnLine(hBoard[3],(byte)0);
		blocks[3] = getSpotOnLine(hBoard[4],(byte)0);
		blocks[4] = getSpotOnLine(hBoard[5],(byte)0);
		for(int i=0;i<4;i++) {
			if(blocks[i]!=0)
				whites += blocks[i]==2 ? 1 : -1;
		}
		...
		whites = 0;
		blocks[1] = getSpotOnLine(hBoard[2],(byte)(dimension-1));
		blocks[2] = getSpotOnLine(hBoard[3],(byte)(dimension-1));
		blocks[3] = getSpotOnLine(hBoard[4],(byte)(dimension-1));
		blocks[4] = getSpotOnLine(hBoard[5],(byte)(dimension-1));
		for(int i=0;i<4;i++) {
			if(blocks[i]!=0)
				whites += blocks[i]==2 ? 1 : -1;
		}
	...
	private float exp(float standard) {
		int num = 0;
		for(int i=0;i<dimension;i++) {
			for(int j=0;j<dimension;j++) {
				short x = getSpotOnLine(hBoard[i],(byte)(j));
				if(x==2||x==3)
					num+=1;
			}
		}
		
		float ans = (float)(1+Math.exp(num-standard));
		return ans;
	}
	...
	public float heuristic() {
		...
		return exp(55)*this.pieceDifferential() +
		   8 * this.moveDifferential() +
		  400 * this.cornerDifferential() +
		   6 *  this.stabilityDifferential() -
		   60 * this.starDifferential()-
		   30 * this.cDifferential()+
		   30*  this.aDifferential()+
		   20 * this.bDifferential()+
		   100 * this.blockDifferential()+
		   winconstant;
		//return this.stabilityDifferential();
	}
